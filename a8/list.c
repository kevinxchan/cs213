...
void list_map1 (void (*f) (element_t*, element_t), struct list* out_list, struct list* in_list) {
	for(int i = 0; i < in_list->len; i++) {
		...
		res[0]=0;
		f(res, in_list->data[i]); 
		list_append(out_list, *res);
    free(res);
	}
}
...
void list_map2 (void (*f) (element_t*, element_t, element_t), struct list* out_list, struct list* in_list0, struct list* in_list1) {
	int outlen = list_len(in_list0) < list_len(in_list1) ? in_list0->len : in_list1->len;
	for (int i = 0; i < outlen; i++) {
		element_t* tmp = malloc(sizeof(element_t));
		tmp[0] = 0;
		f(tmp, in_list0->data[i], in_list1->data[i]);
		list_append(out_list, *tmp);
    free(tmp);
	}
}
