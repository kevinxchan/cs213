...
public class Endianness {
  ...
  public static int bigEndianValue (Byte[] mem) {
      int firstByte = ((mem[0] & 0xFF) << 6 * 4);
      firstByte = firstByte + (((mem[1] & 0xFF) << 4 * 4));
      firstByte = firstByte + (((mem[2] & 0xFF) << 2 * 4));
      firstByte = firstByte + (mem[3] & 0xFF);

    return firstByte;
  }
  
  public static int littleEndianValue (Byte[] mem) {
      int firstByte = ((mem[3] & 0xFF) << 6 * 4);
      firstByte = firstByte + (((mem[2] & 0xFF) << 4 * 4));
      firstByte = firstByte + (((mem[1] & 0xFF) << 2 * 4));
      firstByte = firstByte + (mem[0] & 0xFF);
      return firstByte;
  }
